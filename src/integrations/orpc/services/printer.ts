...
export const printerService = {
	...
	printResumeAsHTML: async (
		input: Pick<InferSelectModel<typeof schema.resume>, "id" | "data" | "userId">,
	): Promise<string> => {
		const { id, data, userId } = input;

		const storageService = getStorageService();
		const htmlPrefix = `uploads/${userId}/html/${id}`;
		await storageService.delete(htmlPrefix);
		...
		try {
			...
			// Inject base tag to ensure relative assets load from the server
