...
function EditorToolbar({ editor, isFullscreen }: { editor: Editor; isFullscreen: boolean }) {
	...
	return (
		<div className="flex flex-wrap items-center gap-y-0.5 rounded-md rounded-b-none border border-b-0">
			...
			<AIOptimizeButton editor={editor} isFullscreen={isFullscreen} />
		</div>
	);
}

function AIOptimizeButton({ editor, isFullscreen }: { editor: Editor; isFullscreen: boolean }) {
	const [isOpen, setIsOpen] = useState(false);
	const [jobDescription, setJobDescription] = useState("");
	const [instruction, setInstruction] = useState("");

	const { mutate: improveContent, isPending } = useMutation(orpc.ai.improveContent.mutationOptions());
	const { provider, apiKey, baseURL, model } = useAIStore();
		...
		improveContent(
			...
			},
			{
				onSuccess: (data) => {
					...
					setIsOpen(false);
					toast.success(t`Content optimized successfully!`);
				},
				onError: (error) => {
	...
	return (
		<>
			<Button
				...
				tabIndex={-1}
				variant="ghost"
				className="rounded-none text-purple-500 hover:bg-purple-50 hover:text-purple-600"
				title={t`Optimize with AI`}
			...
			<Dialog open={isOpen} onOpenChange={setIsOpen}>
				<DialogContent>
					<DialogTitle>
						<Trans>Optimize Content</Trans>
					</DialogTitle>
					<DialogDescription>
						<Trans>
							Use AI to improve your content. You can optionally provide a job description to tailor the result.
						</Trans>
					</DialogDescription>
					...
					<div className="space-y-4">
						...
						<div className="space-y-2">
							...
							<Textarea
								placeholder={t`e.g. Fix grammar, make it more professional, highlight leadership...`}
								value={instruction}
						...
						<div className="flex justify-end gap-2">
							...
							<Button disabled={isPending} onClick={onOptimize}>
