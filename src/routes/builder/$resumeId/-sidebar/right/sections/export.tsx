...
import { CircleNotchIcon, FileHtmlIcon, FileJsIcon, FilePdfIcon } from "@phosphor-icons/react";
...
export function ExportSectionBuilder() {
	...
	const { mutateAsync: printResumeAsHTML, isPending: isPrintingHTML } = useMutation(
		orpc.printer.printResumeAsHTML.mutationOptions(),
	);

	const onDownloadHTML = useCallback(async () => {
		const filename = generateFilename(resume.data.basics.name, "html");
		const toastId = toast.loading(t`Please wait while your HTML resume is being generated...`);

		try {
			const { url } = await printResumeAsHTML({ id: resume.id });
			downloadFromUrl(url, filename);
		} catch {
			toast.error(t`There was a problem while generating the HTML resume, please try again in some time.`);
		} finally {
			toast.dismiss(toastId);
		}
	}, [resume, printResumeAsHTML]);
		...
		<SectionBase type="export" className="space-y-4">
			...
			>
				{isPrintingHTML ? (
					...
					<FileHtmlIcon className="size-6 shrink-0" />
				)}

				<div className="flex flex-1 flex-col gap-y-1">
					<h6 className="font-medium">HTML</h6>
					<p className="text-muted-foreground text-xs leading-normal">
						<Trans>
							Download a copy of your resume in HTML format. This file is a standalone HTML file that you can host on
							your own website.
						</Trans>
					</p>
				</div>
